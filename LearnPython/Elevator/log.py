import os
logfile = "C:\\Users\\onela\\source\\repos\\LearnPython\\LearnPython\\Elevator\\logfile.txt"

def write(logstr):
    writestr = str(logstr)
    # writestr = str(logstr).encode('utf-8')
    try:
        with open(logfile , "a") as file_handler:
            file_handler.write(writestr)
            file_handler.write("\n")
    except IOError:
        print("An IOError has occurred!")
    except AttributeError:
        print("ссылка атрибута или присвоение не могут быть выполнены")
    except TypeError:
        print("функция применяется к объекту несоответствующего типа")
    except ValueError:
        print('ValueError ' + str(BaseException.args))
    except IndexError:
        print('IndexError ' + str(BaseException.args))
    except Exception:
        print("Exception " , str(BaseException.args))
    finally:
        file_handler.close()
        print(logstr)
...
def clear():
    try:
        #os.remove("C:\\Users\\onela\\source\\repos\\LearnPython\\LearnPython\\Elevator\\log.txt")
